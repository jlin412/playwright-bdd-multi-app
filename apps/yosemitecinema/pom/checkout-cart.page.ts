...
export class CheckoutCartPage {
  ...
  @Then('the cart should show {int} line item(s)')
  async checkLineCount(n: number) {
    await expect(this.lineTotals()).toHaveCount(n, { timeout: 15_000 });
  }

  @Then('the grand total should equal the sum of the line totals')
  ...
  // Definite-assignment: these are wired up in _initRedemptions(), not the constructor.
