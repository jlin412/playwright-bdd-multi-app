...
specification `/auto-qa` will implement. Rules:

- Functionality belongs to an **existing feature** → update that feature file.
- Functionality belongs to an **existing API** → update the existing API spec/feature.
- Functionality is **new** → create a new feature/spec file.
- Always attempt to reuse existing feature files and existing step definitions (grep
  `apps/<app>/features/` and the POM/SOM `@Given/@When/@Then` decorators before writing
  a new step).
